...
6. [X]  Configure the system
   1. [X] Generate an [[fstab]]
      ...
      #+END_SRC
   2. [X] copy shiloh-setup to new root directory
      #+BEGIN_SRC sh
...
8. [X] READ https://wiki.archlinux.org/index.php/GRUB2
9. [X] (Optional) Install osprober if Windows dual boot
   #+begin_src sh
     pacman --sync os-prober
   #+end_src
10. [X] run grub-install to cp grub boot files and optionally install to 440-byte MBR boot code region
...
11. [X] Generate GRUB2 BIOS Config file
    #+BEGIN_SRC sh :tangle bin/configure/grub-cfg.sh :shebang #!/bin/bash
      grub-mkconfig -o /boot/grub/grub.cfg
    #+END_SRC
12. [X] Set a root password with passwd.
** TODO Add a user
