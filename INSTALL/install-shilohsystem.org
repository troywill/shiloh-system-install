* Installing Shiloh System step by step Start 8:02 PM Finish 
...
5. [X] Install the base system
   ...
   4. [X] (Optional) Remove symbolic link, and restore pacman pkg directory
	...
	ls -ld /mnt/var/cache/pacman/pkg
      #+end_src
   5. [X] [100%] (Optional) Copy AUR packages
      1. [X] razor-qt 
         #+BEGIN_SRC sh
...
6. [-]  Configure the system
   ...
   10. [ ] Configure /etc/mkinitcpio.conf as needed (see [[mkinitcpio]]) and create an initial RAM disk
       #+BEGIN_SRC sh :tangle bin/configure/mkinitcpio.sh :shebang #!/bin/bash
         mkinitcpio -p linux
       #+END_SRC
7. [0/0] Configure the bootloader: refer back to the appropriate article from the bootloader installation section.
8. [ ] READ https://wiki.archlinux.org/index.php/GRUB2
9. [ ] Install osprober if Windows dual boot
   #+begin_src sh
