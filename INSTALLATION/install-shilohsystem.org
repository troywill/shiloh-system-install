...
3. [ ] Prepare the target media with parted and GPT
   ...
   2. (Optional) Zero drive
	...
	set -o errexit
	DEVICE="/dev/sdX" # find DEVICE with /usr/bin/lsblk and edit this line
        exit
	BS="1M"
	COUNT=4
...
6. [X]  Configure the system
   1. [X] Generate an /etc/fstab file
   ...
   2. [X] copy shiloh-system to new root directory
      ...
      #+END_SRC
   3. [X] chroot into the newly installed system
      #+BEGIN_SRC sh
...
7. [X] Configure the bootloader: refer back to the appropriate article from the bootloader installation section.
   ...
   2. [X] (Optional) Install osprober if Windows dual boot
      #+begin_src sh
